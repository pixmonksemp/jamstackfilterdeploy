...
export function chartSeries(dataa, namesToConsider = []) {
	let finalSeries = []
	let zeroArr = []
	...
	dataa.length && dataa.map(i => {
		if (zeroArr.length != 0) {
			...
			if (namesToConsider && namesToConsider.length > 0) {
				let objectDetails = {}
				namesToConsider.map((names) => {
					if (i.tooltipDetails && i.tooltipDetails[names] && i.tooltipDetails[names].length > 0) {
						Object.assign(objectDetails, {
							[names]: i.tooltipDetails[names].slice(Math.min(...zeroArr))
						})
					}
				})
				Object.assign(value, {
					tooltipDetails: objectDetails
				})
			}
