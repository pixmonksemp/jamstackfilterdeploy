		...
		return (
			<>
				{children && children.length != undefined ? (
					...
					<div
						className='no-option-title-style'
...
export default class SearchableDropdown extends React.Component {
	...
	handleMultiSelectChange = (newValue, actionMeta) => {
		...
		const initialValue =
			preferredRetailer && preferredRetailer.length
				? preferredRetailer
				: defaultValuePassed
		if (newValue && newValue.length == 0 && this.props.isFreeHandSelect) {
			onChange(newValue, 'empty')
		} else {
			if (
	...
	handleChange = (item) => {
		...
		if (item == null && this.props.isFreeHandSelect) {
			this.setState({
				singleSelectValue: [],
	...
	render() {
		...
		return (
			<div
				className={className}
				title={tooltipName && tooltipName.length > 0 ? [...new Set(tooltipName)] : tooltipName}
				id={
